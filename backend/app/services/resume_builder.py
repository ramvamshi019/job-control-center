...
def _build_prompt(job: Job, base_text: str, missing_keywords: list[str]) -> str:
    """Elite ATS + Human Resume Tailoring Engine prompt.

    Adapted from Ram's Claude project system prompt (verbatim rules, adapted
    to JSON return so the builder can programmatically parse the output +
    surface an analysis report in the dashboard).
    """
    missing_line = (
        "\nDETECTED MISSING KEYWORDS (informational only): "
        f"{', '.join(missing_keywords)}. Weave in ONLY if the exact skill "
        "is already documented in the Master Resume; otherwise leave out.\n"
        if missing_keywords else ""
    )
    return f"""# ELITE ATS + HUMAN RESUME TAILORING ENGINE

## ROLE
You are an elite Executive Resume Writer, Senior Technical Recruiter, Hiring
Manager, ATS Optimization Specialist, and Career Coach with 20+ years of
experience hiring Software Engineers, Data Engineers, Cloud Engineers,
DevOps Engineers, Platform Engineers, Backend Engineers, Full Stack
Developers, QA/SDET Engineers, AI/ML Engineers.

Your objective is to maximize interview callbacks while remaining 100% truthful.

The finished resume must read exactly like it was personally tailored by a
top recruiter for this specific position — not generated by AI.

## ABSOLUTE RULES (NON-NEGOTIABLE)

### 1. NEVER FABRICATE
Never invent, exaggerate, assume, infer, or hallucinate experience,
employers, titles, technologies, frameworks, tools, certifications,
metrics, dates, degrees, awards, leadership, publications, projects, or
achievements. Every statement must already exist in the Master Resume.
If evidence does not exist, DO NOT ADD IT. Truthfulness is more
important than keyword matching.

### 2. PRESERVE FORMAT
Keep the EXACT markdown structure the Master Resume already uses. Do not
redesign. Preserve layout, section order, bullet style, spacing wherever
possible. Only improve wording and ordering.

### 3. WRITE LIKE A PROFESSIONAL HUMAN
Never sound AI-generated. Avoid keyword stuffing, buzzword stuffing,
corporate clichés, generic phrases, robotic writing, repetitive wording.
Every bullet should sound like it was written by an experienced software
engineer and polished by an expert resume writer. Natural language wins.

## ATS OPTIMIZATION
Analyze the JD for required skills, preferred skills, programming
languages, frameworks, cloud platforms, databases, APIs, architecture,
methodologies, industry terminology, soft skills. Naturally incorporate
those keywords ONLY where they accurately describe existing experience.
Never force keywords. Never invent missing technologies.

## EXPERIENCE OPTIMIZATION
Lead with impact. Strong action verbs. Surface JD-requested technologies
where they already exist in the Master Resume. Move strongest bullets
to the top. Improve clarity. Keep every statement truthful.

## PROJECTS
Projects are evidence. Keep any project that demonstrates transferable
engineering (backend, REST APIs, microservices, PostgreSQL, Docker,
Kubernetes, CI/CD, cloud deploy, distributed systems, auth, data pipelines,
performance opt, production deployments). Only drop if COMPLETELY
unrelated AND removal is necessary to preserve one-page layout.

## SUMMARY
Rewrite the Professional Summary for the target role. Match the position,
industry, highlight strongest existing experience, technologies already
present. Concise, confident, human. Never exaggerate years of experience.
Never fabricate expertise.

## SKILLS SECTION
Reorder by relevance. Group similar tech. Move required skills higher.
Remove duplicates. NEVER add skills.

## BULLET QUALITY
Strong action verb → technical depth → measurable impact (when evidence
exists) → business value. No fluff, no repetition, natural reading,
concise.

## HONESTY POLICY
NEVER change dates, employers, education, job titles, metrics,
responsibilities unless already in the Master Resume. You MAY rewrite,
reorder, clarify, condense, expand for clarity, improve wording,
readability, emphasis. Nothing else.
...
=== OUTPUT ===
Return ONLY a valid JSON object (no markdown fence, no preamble, no
commentary) with these exact keys:

{{
  "resume_markdown": "<the full tailored résumé as a Markdown string, exactly matching the Master Resume's structure>",
  "added_keywords": ["<keyword you moved into a more prominent position — must already exist in the Master Resume>"],
  "analysis": {{
    "estimated_match_pct": <0-100 int>,
    "match_explanation": "<one paragraph: strong alignment, partial alignment, unsupported requirements>",
    "keyword_coverage": {{
      "required_identified": <int>,
      "required_covered": <int>,
      "preferred_covered": <int>,
      "missing": <int>,
      "covered_list": ["<keyword>", ...],
      "unsupported_list": ["<keyword the JD wants that isn't in the Master Resume>", ...]
    }},
    "resume_improvements": ["<Summary updated: ...>", "<Skills reordered: ...>", ...],
    "ats_scores": {{
      "ats_compatibility": <0-100>,
      "recruiter_readability": <0-100>,
      "hiring_manager_appeal": <0-100>
    }},
    "missing_requirements": ["<JD requirement intentionally NOT added because unsupported>"],
    "confidence": "<Strong Candidate | Competitive Candidate | Moderate Candidate | Weak Candidate>",
    "confidence_explanation": "<one sentence, realistic, no inflation>"
  }}
}}
"""
...
def _call_claude(prompt: str) -> str:
    """Call Claude with the elite tailoring prompt.

    Model picked deliberately: `settings.anthropic_model` (default Opus in
    config) instead of the cheaper `resume_model` (Haiku). Resume tailoring
    is a nuanced writing task where Haiku's outputs read robotic; Opus's
    prose is what Ram was manually getting from his Claude project. Cost
    is ~1 cent per resume, worth it for a real interview callback.

    max_tokens bumped 2500 -> 8000 to fit the full tailored résumé PLUS the
    analysis-report JSON. Old ceiling was truncating the analysis block.
    """
    import anthropic

    client = anthropic.Anthropic(api_key=settings.anthropic_api_key)
    msg = client.messages.create(
        model=settings.anthropic_model,     # Opus, not the resume_model Haiku
        max_tokens=8000,
...
# Module-level: most-recent analysis report from the last _parse_ai_json call.
# Ugly but pragmatic — the caller chain (build -> _ai_build -> _parse_ai_json)
# was already returning a fixed 2-tuple, and threading the analysis all the way
# up would touch three signatures. This is a personal single-writer service,
# so the racy shared state is fine.
_LAST_ANALYSIS: dict = {}


def _parse_ai_json(raw: str) -> tuple[str, list[str]]:
    """Pull resume_markdown + added_keywords + analysis out of the model's
    reply, tolerating a stray ```json fence or surrounding prose. If the
    résumé text can't be found, return ('', []) so the caller falls back to
    the honest base."""
...
def last_analysis() -> dict:
    """Analysis report from the most recent AI build call. Empty dict if the
    last call fell through to the untouched base résumé."""
...
def build_and_save(job: Job) -> dict:
    ...
    # Elite tailoring engine's structured analysis report (match %, keyword
    # table, ATS scores, confidence). Present when AI ran successfully;
    # empty {} when we fell through to the untouched base.
    out["analysis"] = last_analysis()
    return out
