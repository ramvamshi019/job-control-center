...
# The tailoring rules live in resumes/tailor_system_prompt.md so Ram can iterate
# on wording without touching Python or redeploying. Loaded fresh on every build
# request — cheap (a few KB), and edits take effect on the next tailor call.
TAILOR_PROMPT_PATH = RESUMES_DIR / "tailor_system_prompt.md"


def _load_tailor_rules() -> str:
    """Return the file-driven Rule 0-4 body from tailor_system_prompt.md.
    Empty string on any read failure so the wrapper degrades to a minimal
    honesty-first fallback (never fabricates, just doesn't have Ram's voice)."""
    try:
        return TAILOR_PROMPT_PATH.read_text(encoding="utf-8").strip()
    except Exception as exc:  # noqa: BLE001
        log.warning("tailor_system_prompt.md unreadable, using fallback: %s", exc)
        return (
            "Tailor the base résumé to the JD. Never fabricate. Preserve the base's "
            "exact structure, headings, bullet count, and voice. Reorder to surface "
            "JD-matching experience first."
        )


def _build_prompt(job: Job, base_text: str, missing_keywords: list[str]) -> str:
    """Wrap Ram's file-driven Rule 0-4 body with the JD/base inputs and a JSON
    output contract so _parse_ai_json can pick up the tailored markdown plus
    the gap report.

    The rules themselves live in resumes/tailor_system_prompt.md — edit that
    file, not this function. This wrapper only supplies the inputs the rules
    expect (BASE_RESUME, JD) and the machine-parseable output shape.
    """
    rules = _load_tailor_rules()
    missing_line = (
        "\nDETECTED JD KEYWORDS NOT IN BASE (informational): "
        f"{', '.join(missing_keywords)}. Per Rule 2, list any un-backed ones "
        "in the gap report rather than injecting them into the résumé.\n"
...
=== BASE_RESUME (the only source of truth) ===
{base_text}

=== OUTPUT CONTRACT ===
Return ONLY a valid JSON object (no markdown fence, no preamble, no
commentary). If your overall verdict is "skip", set resume_markdown to ""
and stop — the caller will fall back to the untouched base.

{{
  "resume_markdown": "<the full tailored résumé as a Markdown string, matching BASE_RESUME's structure exactly per Rule 0. Empty string if overall==skip>",
  "added_keywords": ["<JD term you surfaced into a more prominent position — must be genuinely backed by base experience>"],
  "analysis": {{
    "overall": "<strong_match | stretch | skip>",
    "gaps": [
      {{"requirement":"","centrality":"central|peripheral","classification":"met|adjacent|dealbreaker","path":"","time_to_credible":"","verdict":""}}
    ],
    "keywords_injected": ["<verbatim JD term that now appears in the résumé>"],
    "reframes": ["<how an adjacent skill was bridged in the résumé>"],
    "needs_metric": ["<qualitative bullet the candidate should back-fill with a real number>"],
    "estimated_match_pct": <0-100 int>,
    "match_explanation": "<one paragraph>",
    "keyword_coverage": {{
      ...
      "unsupported_list": ["<JD keyword absent from base>", ...]
    }},
    "resume_improvements": ["<Summary reordered: ...>", "<Skills reordered: ...>", ...],
    "ats_scores": {{
    ...
    "confidence_explanation": "<one sentence, realistic>"
  }}
}}
"""
