...
function StatusPill({ status }: { status: JobStatus }) {
  const map: Record<JobStatus, { label: string; variant: React.ComponentProps<typeof Badge>["variant"] }> = {
...
export default function BestMatches() {
  ...
  // Optimistic mutation: flip the row's status in the cache immediately, then
  // fire the API call. If it fails, roll back. Never wait for the network to
  // update the UI -- that's the "feels fast" property Streamlit couldn't give.
  ...
  const cols = useMemo(
    () => [
      ...
      columnHelper.accessor("discovered_at", {
        ...
        size: 70,
        cell: (info) => (
          <span className="text-xs text-muted-foreground">{daysAgo(info.getValue())}</span>
        ),
      }),
      ...
      <div className="border-b border-border px-6 py-4">
        <div className="flex items-baseline justify-between">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-semibold">Best Matches</h1>
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Full applyable backlog — ranked by match score. Sponsor-safe, US, on-target roles you
          haven&apos;t applied to yet. Press{" "}
          <kbd className="rounded border border-border bg-muted px-1 text-[10px]">j</kbd>/
          <kbd className="rounded border border-border bg-muted px-1 text-[10px]">k</kbd> to move,{" "}
          <kbd className="rounded border border-border bg-muted px-1 text-[10px]">a</kbd> to apply,{" "}
          <kbd className="rounded border border-border bg-muted px-1 text-[10px]">x</kbd> to archive.
        </p>
      </div>
      ...
      <div ref={parentRef} className="relative flex-1 overflow-auto">
        {error ? (
          <div className="flex h-full items-center justify-center text-sm text-destructive">
            Backend unreachable — check the SSH tunnel: <code className="ml-2">ssh -L 8000:localhost:8000 root@143.198.188.116</code>
          </div>
        ) : isLoading ? (
          ...
          <div className="flex h-full items-center justify-center text-sm text-muted-foreground">
            No matches — lower the min score or widen the freshness window.
          </div>
        ) : (
...
function Kpi({
  ...
  const color =
    tone === "sponsor" ? "text-emerald-400" : tone === "new" ? "text-primary" : "text-foreground"
  return (
    <div className="flex items-baseline gap-1.5">
      <span className={cn("tabular-nums text-lg font-semibold", color)}>{value.toLocaleString()}</span>
